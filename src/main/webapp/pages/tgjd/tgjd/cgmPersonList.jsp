...
<script type="text/javascript" language="JavaScript">
	...
	{
		var url = "tgjd/CgmPersonAction!showDetail.action" + "?cgmPerson.personId=" + personId;
		window.showModalDialog(url,window,'dialogWidth=800px;dialogHeight=600px;');
	}
	//打开修改页面
	function cgmPersonUpdate(personId)
	{
		var url = "<%=request.getContextPath()%>/tgjd/CgmPersonAction!prepareUpdate.action" + "?cgmPerson.personId=" + personId;
		window.showModalDialog(url,window,'dialogWidth=800px;dialogHeight=600px;');
	}	
	//删除
	function cgmPersonDelete(personId) {
		dt$.confirm("确认删除吗？", "询问", function(msgBtn) {
			if (msgBtn == dt$.DR_OK) {
				var url = "tgjd/CgmPersonAction!delete.action";
...
<dt:grid id="listGrid" url="tgjd/CgmPersonAction!retrieve.action?'+$('#parameters').serialize()+'" showPage="true" width="100%" height="100%" shrinkToFit="true" caption="标题">
