...
<script type="text/javascript" language="JavaScript">
	...
	{
		var url = "test/AppVideoAction!showDetail.action" + "?appVideo.appVideoId=" + appVideoId;
		window.showModalDialog(url,window,'dialogWidth=800px;dialogHeight=600px;');
	}
	//打开修改页面
	function appVideoUpdate(appVideoId)
	{
		var url = "<%=request.getContextPath()%>/test/AppVideoAction!prepareUpdate.action" + "?appVideo.appVideoId=" + appVideoId;
		window.showModalDialog(url,window,'dialogWidth=800px;dialogHeight=600px;');
	}	
	//删除
	function appVideoDelete(appVideoId) {
		dt$.confirm("确认删除吗？", "询问", function(msgBtn) {
			if (msgBtn == dt$.DR_OK) {
				var url = "test/AppVideoAction!delete.action";
...
<dt:grid id="listGrid" url="test/AppVideoAction!retrieve.action?'+$('#parameters').serialize()+'" showPage="true" width="100%" height="100%" shrinkToFit="true" caption="标题">
