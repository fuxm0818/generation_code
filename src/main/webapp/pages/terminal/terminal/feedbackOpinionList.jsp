...
<script type="text/javascript" language="JavaScript">
	...
	{
		var url = "terminal/FeedbackOpinionAction!showDetail.action" + "?feedbackOpinion.feedbackOpinion=" + feedbackOpinion;
		window.showModalDialog(url,window,'dialogWidth=800px;dialogHeight=600px;');
	}
	//打开修改页面
	function feedbackOpinionUpdate(feedbackOpinion)
	{
		var url = "<%=request.getContextPath()%>/terminal/FeedbackOpinionAction!prepareUpdate.action" + "?feedbackOpinion.feedbackOpinion=" + feedbackOpinion;
		window.showModalDialog(url,window,'dialogWidth=800px;dialogHeight=600px;');
	}	
	//删除
	function feedbackOpinionDelete(feedbackOpinion) {
		dt$.confirm("确认删除吗？", "询问", function(msgBtn) {
			if (msgBtn == dt$.DR_OK) {
				var url = "terminal/FeedbackOpinionAction!delete.action";
...
<dt:grid id="listGrid" url="terminal/FeedbackOpinionAction!retrieve.action?'+$('#parameters').serialize()+'" showPage="true" width="100%" height="100%" shrinkToFit="true" caption="标题">
